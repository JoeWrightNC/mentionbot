...
module.exports = function DewittDaily() {
    //Daily Serve of Content to Slack
    var reqDewitt = request('https://www.google.com/alerts/feeds/13227863141014072795/9428849158565112356')
    var feedparser = new FeedParser([]);

    reqDewitt.on('error', function (error) {
        console.log("Dewitt Req Error")
        console.log(error)
    })

    reqDewitt.on('response', function(res) {
        var streamDewitt = this;

        if (res.statusCode !== 200) {
            this.emit('error', new Error('Bad status code'))
        } else {
            streamDewitt.pipe(feedparser);
        }
    });
    ...
    feedparser.on('readable', function () {
        ...
        while (item = stream.read()) {        
            ...
            if (dateCheckServer === dateCheckFeedItem && domainCleaned !== 'https://ohiocapitaljournal') {

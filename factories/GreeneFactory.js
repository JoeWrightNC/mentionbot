...
module.exports = function GreeneDaily() {
    //Daily Serve of Content to Slack
    var reqGreene = request('https://www.google.com/alerts/feeds/13227863141014072795/6183237802050646518')
    var feedparser = new FeedParser([]);

    reqGreene.on('error', function (error) {
        console.log("AZ Req Error")
        console.log(error)
    })

    reqGreene.on('response', function(res) {
        var streamGreene = this;

        if (res.statusCode !== 200) {
            this.emit('error', new Error('Bad status code'))
        } else {
            streamGreene.pipe(feedparser);
        }
    });

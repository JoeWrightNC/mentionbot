...
module.exports = function MIDaily() {
    //Daily Serve of Content to Slack
    var reqMI = request('https://www.google.com/alerts/feeds/13227863141014072795/1120155205217899434')
    var feedparser = new FeedParser([]);

    reqMI.on('error', function (error) {
        console.log("MI Req Error")
        console.log(error)
    })

    reqMI.on('response', function(res) {
        var streamMI = this;

        if (res.statusCode !== 200) {
            this.emit('error', new Error('Bad status code'))
        } else {
            streamMI.pipe(feedparser);
        }
    });

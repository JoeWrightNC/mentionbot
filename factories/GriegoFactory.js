...
module.exports = function GriegoDaily() {
    //Daily Serve of Content to Slack
    var reqGriego = request('https://www.google.com/alerts/feeds/13227863141014072795/3665361128141157391')
    var feedparser = new FeedParser([]);

    reqGriego.on('error', function (error) {
        console.log("AZ Req Error")
        console.log(error)
    })

    reqGriego.on('response', function(res) {
        var streamGriego = this;

        if (res.statusCode !== 200) {
            this.emit('error', new Error('Bad status code'))
        } else {
            streamGriego.pipe(feedparser);
        }
    });

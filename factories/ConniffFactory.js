...
module.exports = function ConniffDaily() {
    //Daily Serve of Content to Slack
    var reqConniff = request('https://www.google.com/alerts/feeds/13227863141014072795/4575529955700518282')
    var feedparser = new FeedParser([]);

    reqConniff.on('error', function (error) {
        console.log("AZ Req Error")
        console.log(error)
    })

    reqConniff.on('response', function(res) {
        var streamConniff = this;

        if (res.statusCode !== 200) {
            this.emit('error', new Error('Bad status code'))
        } else {
            streamConniff.pipe(feedparser);
        }
    });
    ...
    feedparser.on('readable', function () {
        ...
        while (item = stream.read()) {        
            ...
            if (dateCheckServer === dateCheckFeedItem && domainCleaned !== 'https://wisconsinexaminer') {

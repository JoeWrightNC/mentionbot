...
module.exports = function MinnesotaDaily() {
    //Daily Serve of Content to Slack
    var reqMinnesota = request('https://www.google.com/alerts/feeds/13227863141014072795/16036078473434678625')
    var feedparser = new FeedParser([]);

    reqMinnesota.on('error', function (error) {
        console.log("AZ Req Error")
        console.log(error)
    })

    reqMinnesota.on('response', function(res) {
        var streamMinnesota = this;

        if (res.statusCode !== 200) {
            this.emit('error', new Error('Bad status code'))
        } else {
            streamMinnesota.pipe(feedparser);
        }
    });
    ...
    feedparser.on('readable', function () {
        ...
        while (item = stream.read()) {        
            ...
            if (dateCheckServer === dateCheckFeedItem && domainCleaned !== 'https://minnesotareformer') {

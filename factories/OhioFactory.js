...
module.exports = function OHDaily() {
    //Daily Serve of Content to Slack
    var reqOH = request('https://www.google.com/alerts/feeds/13227863141014072795/6147203869375309212')
    var feedparser = new FeedParser([]);

    reqOH.on('error', function (error) {
        console.log("OH Req Error")
        console.log(error)
    })

    reqOH.on('response', function(res) {
        var streamOH = this;

        if (res.statusCode !== 200) {
            this.emit('error', new Error('Bad status code'))
        } else {
            streamOH.pipe(feedparser);
        }
    });

...
module.exports = function GADaily() {
    //Daily Serve of Content to Slack
    var reqGA = request('https://www.google.com/alerts/feeds/13227863141014072795/3936703063846399134')
    var feedparser = new FeedParser([]);

    reqGA.on('error', function (error) {
        console.log("GA Req Error")
        console.log(error)
    })

    reqGA.on('response', function(res) {
        var streamGA = this;

        if (res.statusCode !== 200) {
            this.emit('error', new Error('Bad status code'))
        } else {
            streamGA.pipe(feedparser);
        }
    });

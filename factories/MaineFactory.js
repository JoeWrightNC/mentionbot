...
module.exports = function MEDaily() {
    //Daily Serve of Content to Slack
    var reqME = request('https://www.google.com/alerts/feeds/13227863141014072795/3207807947249219446')
    var feedparser = new FeedParser([]);

    reqME.on('error', function (error) {
        console.log("ME Req Error")
        console.log(error)
    })

    reqME.on('response', function(res) {
        var streamME = this;

        if (res.statusCode !== 200) {
            this.emit('error', new Error('Bad status code'))
        } else {
            streamME.pipe(feedparser);
        }
    });

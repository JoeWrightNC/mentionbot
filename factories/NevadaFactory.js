...
module.exports = function NVDaily() {
    //Daily Serve of Content to Slack
    var reqNV = request('https://www.google.com/alerts/feeds/13227863141014072795/8802959803756519930')
    var feedparser = new FeedParser([]);

    reqNV.on('error', function (error) {
        console.log("NV Req Error")
        console.log(error)
    })

    reqNV.on('response', function(res) {
        var streamNV = this;

        if (res.statusCode !== 200) {
            this.emit('error', new Error('Bad status code'))
        } else {
            streamNV.pipe(feedparser);
        }
    });

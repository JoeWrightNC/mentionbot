...
module.exports = function FitzsimonDaily() {
    //Daily Serve of Content to Slack
    var reqFitzsimon = request('https://www.google.com/alerts/feeds/13227863141014072795/16019957683790464711')
    var feedparser = new FeedParser([]);

    reqFitzsimon.on('error', function (error) {
        console.log("AZ Req Error")
        console.log(error)
    })

    reqFitzsimon.on('response', function(res) {
        var streamFitzsimon = this;

        if (res.statusCode !== 200) {
            this.emit('error', new Error('Bad status code'))
        } else {
            streamFitzsimon.pipe(feedparser);
        }
    });

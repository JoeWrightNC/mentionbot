...
module.exports = function ObradovichDaily() {
    //Daily Serve of Content to Slack
    var reqObradovich = request('https://www.google.com/alerts/feeds/13227863141014072795/16415676789723663764')
    var feedparser = new FeedParser([]);

    reqObradovich.on('error', function (error) {
        console.log("AZ Req Error")
        console.log(error)
    })

    reqObradovich.on('response', function(res) {
        var streamObradovich = this;

        if (res.statusCode !== 200) {
            this.emit('error', new Error('Bad status code'))
        } else {
            streamObradovich.pipe(feedparser);
        }
    });

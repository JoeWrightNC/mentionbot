...
module.exports = function VADaily() {
    //Daily Serve of Content to Slack
    var reqVA = request('https://www.google.com/alerts/feeds/13227863141014072795/17974945801607592934')
    var feedparser = new FeedParser([]);

    reqVA.on('error', function (error) {
        console.log("VA Req Error")
        console.log(error)
    })

    reqVA.on('response', function(res) {
        var streamVA = this;

        if (res.statusCode !== 200) {
            this.emit('error', new Error('Bad status code'))
        } else {
            streamVA.pipe(feedparser);
        }
    });

...
module.exports = function PADaily() {
    //Daily Serve of Content to Slack
    var reqPA = request('https://www.google.com/alerts/feeds/13227863141014072795/14718862961943720382')
    var feedparser = new FeedParser([]);

    reqPA.on('error', function (error) {
        console.log("PA Req Error")
        console.log(error)
    })

    reqPA.on('response', function(res) {
        var streamPA = this;

        if (res.statusCode !== 200) {
            this.emit('error', new Error('Bad status code'))
        } else {
            streamPA.pipe(feedparser);
        }
    });
    ...
    feedparser.on('readable', function () {
        ...
        while (item = stream.read()) {        
            ...
            if (dateCheckServer === dateCheckFeedItem && domainCleaned !== 'https://www.penncapital-star') {

...
module.exports = function MccauleyDaily() {
    //Daily Serve of Content to Slack
    var reqMccauley = request('https://www.google.com/alerts/feeds/13227863141014072795/5486021936656916068')
    var feedparser = new FeedParser([]);

    reqMccauley.on('error', function (error) {
        console.log("AZ Req Error")
        console.log(error)
    })

    reqMccauley.on('response', function(res) {
        var streamMccauley = this;

        if (res.statusCode !== 200) {
            this.emit('error', new Error('Bad status code'))
        } else {
            streamMccauley.pipe(feedparser);
        }
    });
    ...
    feedparser.on('readable', function () {
        ...
        while (item = stream.read()) {        
            ...
            if (dateCheckServer === dateCheckFeedItem && domainCleaned !== 'https://mainebeacon') {

...
module.exports = function CoolicanDaily() {
    //Daily Serve of Content to Slack
    var reqCoolican = request('https://www.google.com/alerts/feeds/13227863141014072795/11759548277480291108')
    var feedparser = new FeedParser([]);

    reqCoolican.on('error', function (error) {
        console.log("AZ Req Error")
        console.log(error)
    })

    reqCoolican.on('response', function(res) {
        var streamCoolican = this;

        if (res.statusCode !== 200) {
            this.emit('error', new Error('Bad status code'))
        } else {
            streamCoolican.pipe(feedparser);
        }
    });

...
module.exports = function KurtzDaily() {
    //Daily Serve of Content to Slack
    var reqKurtz = request('https://www.google.com/alerts/feeds/13227863141014072795/15840998122427405441')
    var feedparser = new FeedParser([]);

    reqKurtz.on('error', function (error) {
        console.log("AZ Req Error")
        console.log(error)
    })

    reqKurtz.on('response', function(res) {
        var streamKurtz = this;

        if (res.statusCode !== 200) {
            this.emit('error', new Error('Bad status code'))
        } else {
            streamKurtz.pipe(feedparser);
        }
    });
    ...
    feedparser.on('readable', function () {
        ...
        while (item = stream.read()) {        
            ...
            if (dateCheckServer === dateCheckFeedItem && domainCleaned !== 'https://www.marylandmatters') { 

...
module.exports = function DemasDaily() {
    //Daily Serve of Content to Slack
    var reqDemas = request('https://www.google.com/alerts/feeds/13227863141014072795/9453446947022039060')
    var feedparser = new FeedParser([]);

    reqDemas.on('error', function (error) {
        console.log("AZ Req Error")
        console.log(error)
    })

    reqDemas.on('response', function(res) {
        var streamDemas = this;

        if (res.statusCode !== 200) {
            this.emit('error', new Error('Bad status code'))
        } else {
            streamDemas.pipe(feedparser);
        }
    });
    ...
    feedparser.on('readable', function () {
        ...
        while (item = stream.read()) {        
            ...
            if (dateCheckServer === dateCheckFeedItem && domainCleaned !== 'https://www.michiganadvance') {

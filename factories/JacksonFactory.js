...
module.exports = function JacksonDaily() {
    //Daily Serve of Content to Slack
    var reqJackson = request('https://www.google.com/alerts/feeds/13227863141014072795/1817496350134730700')
    var feedparser = new FeedParser([]);

    reqJackson.on('error', function (error) {
        console.log("AZ Req Error")
        console.log(error)
    })

    reqJackson.on('response', function(res) {
        var streamJackson = this;

        if (res.statusCode !== 200) {
            this.emit('error', new Error('Bad status code'))
        } else {
            streamJackson.pipe(feedparser);
        }
    });
    ...
    feedparser.on('readable', function () {
        ...
        while (item = stream.read()) {        
            ...
            if (dateCheckServer === dateCheckFeedItem && domainCleaned !== 'https://www.nevadacurrent') {

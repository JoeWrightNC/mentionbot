...
module.exports = function TippingDaily() {
    //Daily Serve of Content to Slack
    var reqTipping = request('https://www.google.com/alerts/feeds/13227863141014072795/5964846474334316380')
    var feedparser = new FeedParser([]);

    reqTipping.on('error', function (error) {
        console.log("AZ Req Error")
        console.log(error)
    })

    reqTipping.on('response', function(res) {
        var streamTipping = this;

        if (res.statusCode !== 200) {
            this.emit('error', new Error('Bad status code'))
        } else {
            streamTipping.pipe(feedparser);
        }
    });

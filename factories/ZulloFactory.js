...
module.exports = function ZulloDaily() {
    //Daily Serve of Content to Slack
    var reqZullo = request('https://www.google.com/alerts/feeds/13227863141014072795/1772082451284234106')
    var feedparser = new FeedParser([]);

    reqZullo.on('error', function (error) {
        console.log("AZ Req Error")
        console.log(error)
    })

    reqZullo.on('response', function(res) {
        var streamZullo = this;

        if (res.statusCode !== 200) {
            this.emit('error', new Error('Bad status code'))
        } else {
            streamZullo.pipe(feedparser);
        }
    });
    ...
    feedparser.on('readable', function () {
        ...
        while (item = stream.read()) {        
            ...
            if (dateCheckServer === dateCheckFeedItem && domainCleaned !== 'https://www.virginiamercury') {

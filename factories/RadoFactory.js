...
module.exports = function RadoDaily() {
    //Daily Serve of Content to Slack
    var reqRado = request('https://www.google.com/alerts/feeds/13227863141014072795/12441280250484758479')
    var feedparser = new FeedParser([]);

    reqRado.on('error', function (error) {
        console.log("AZ Req Error")
        console.log(error)
    })

    reqRado.on('response', function(res) {
        var streamRado = this;

        if (res.statusCode !== 200) {
            this.emit('error', new Error('Bad status code'))
        } else {
            streamRado.pipe(feedparser);
        }
    });

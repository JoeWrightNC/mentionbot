...
module.exports = function WIDaily() {
    //Daily Serve of Content to Slack
    var reqWI = request('https://www.google.com/alerts/feeds/13227863141014072795/2839737301921072458')
    var feedparser = new FeedParser([]);

    reqWI.on('error', function (error) {
        console.log("WI Req Error")
        console.log(error)
    })

    reqWI.on('response', function(res) {
        var streamWI = this;

        if (res.statusCode !== 200) {
            this.emit('error', new Error('Bad status code'))
        } else {
            streamWI.pipe(feedparser);
        }
    });

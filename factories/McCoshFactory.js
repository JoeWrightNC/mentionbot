...
module.exports = function McCoshDaily() {
    //Daily Serve of Content to Slack
    var reqMcCosh = request('https://www.google.com/alerts/feeds/13227863141014072795/15840998122427407225')
    var feedparser = new FeedParser([]);

    reqMcCosh.on('error', function (error) {
        console.log("AZ Req Error")
        console.log(error)
    })

    reqMcCosh.on('response', function(res) {
        var streamMcCosh = this;

        if (res.statusCode !== 200) {
            this.emit('error', new Error('Bad status code'))
        } else {
            streamMcCosh.pipe(feedparser);
        }
    });

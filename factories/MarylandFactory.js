...
module.exports = function MDDaily() {
    //Daily Serve of Content to Slack
    var reqMD = request('https://www.google.com/alerts/feeds/13227863141014072795/17059704821431475666')
    var feedparser = new FeedParser([]);

    reqMD.on('error', function (error) {
        console.log("MD Req Error")
        console.log(error)
    })

    reqMD.on('response', function(res) {
        var streamMD = this;

        if (res.statusCode !== 200) {
            this.emit('error', new Error('Bad status code'))
        } else {
            streamMD.pipe(feedparser);
        }
    });
